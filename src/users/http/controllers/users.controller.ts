...
export class UsersController {
  ...
  async findOne(@Param('id') id: string): Promise<UserHttpResponse> {
    ...
    const userResponse = new UserHttpResponse(user.id, user.email, user.firstName, user.lastName)
    return userResponse;
  }

  @Post('')
  async create(@Body() createUserHttpRequest: CreateUserHttpRequest): Promise<UserHttpResponse> {
    const createUserCommand = new CreateUserCommand(createUserHttpRequest.email, createUserHttpRequest.firstName, createUserHttpRequest.lastName);
  ...
  @Post(':id')
  async edit(@Param('id') id: string, @Body() editUserHttpRequest: EditUserHttpRequest): Promise<UserHttpResponse> {
    const user: User = await this.usersService.edit(id, editUserHttpRequest.email, editUserHttpRequest.firstName, editUserHttpRequest.lastName);
