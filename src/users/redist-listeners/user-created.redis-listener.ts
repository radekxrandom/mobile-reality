...
export class UserCreatedRedisListener{
  constructor(private readonly redisClient: RedisClient, private readonly usersService: UsersService){}

  listenToRedis() {
    this.redisClient.on('message', async (channel, message) => {
      const userProperties = JSON.parse(message)
      const {email, firstName, lastName} = userProperties
      const createUserCommand = new CreateUserCommand(email, firstName, lastName);
